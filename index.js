...
const caseHandler = require('./system/caseHandler');
...
async function startBot() {
    ...
    sock.ev.on('messages.upsert', async chatUpdate => {
        try {
            ...
            // ---- [ SINKRONISASI COCOK NOMOR AGAR AMAN DI MODE SELF ] ----
            let senderNumber = m.sender.split('@')[0]; 
            let botNumber = sock.user.id.split(':')[0].split('@')[0]; 

            let isOwner = [botNumber, ...global.owner].map(v => v.replace(/[^0-9]/g, '')).includes(senderNumber);
            let isPremium = isOwner || global.premium.map(v => v.replace(/[^0-9]/g, '')).includes(senderNumber);
            let isBotAdmin = groupAdmins.includes(sock.decodeJid(sock.user.id));
            let isAdmin = groupAdmins.includes(m.sender);

            // ---- [ SEKAT UTAMA SELF / PUBLIC ] ----
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            if (fs.existsSync('./restarting.json')) {
                try {
                    const restartData = JSON.parse(fs.readFileSync('./restarting.json', 'utf8'));
                    await sock.sendMessage(restartData.chatId, { 
                        text: `✅ *Bot Berhasil Online Kembali!*\nProses restart selesai.` 
                    }, { quoted: restartData.msg });
                    fs.unlinkSync('./restarting.json');
                } catch (e) {
                    console.error("Gagal memproses auto-pemberitahuan restart:", e);
                }
            }
        }
    });
